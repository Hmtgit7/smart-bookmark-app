    ...
    const [messages, setMessages] = useState<Message[]>([
        {
            role: 'assistant',
            content:
                "Hi! 👋 I'm your bookmark assistant. Ask me anything like:\n\n• 'Find my React tutorials'\n• 'Show bookmarks from last week'\n• 'What's in my Learning category?'",
            timestamp: new Date(),
        },
    ]);
    const [input, setInput] = useState('');
    ...
    return (
        <>
                        ...
                        <div className="h-full overflow-y-auto px-4 sm:px-6" ref={scrollRef}>
                            <div className="space-y-3 sm:space-y-4 py-3 sm:py-4">
                                {messages.map((message, index) => (
                                    <div
                                        key={index}
                                        className={`flex gap-2 sm:gap-3 animate-in slide-in-from-bottom-2 fade-in duration-300 ${
                                            message.role === 'user'
                                                ? 'justify-end'
                                                : 'justify-start'
                                        }`}
                                    >
                                        ...
                                        <div
                                            className={`rounded-2xl px-3 py-2 max-w-[85%] sm:max-w-[80%] ${
                                                message.role === 'user'
            ...
            <Button
                ...
                size="lg"
                className={`fixed bottom-6 right-6 h-14 w-14 sm:h-16 sm:w-16 rounded-full shadow-2xl z-50 transition-all duration-300 ${
                    open
                        ? 'rotate-90 scale-90'
